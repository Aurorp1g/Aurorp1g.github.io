document.addEventListener('DOMContentLoaded', function() {
    ...
    window.initializeAnimation = function() {
        ...
        // 计算缩放比例
        var scale = Math.min(containerWidth / baseSize, containerHeight / baseSize) * pixelRatio;
        
        // 设置物理尺寸
        canvasWidth = canvas.width = baseSize * scale;
        canvasHeight = canvas.height = baseSize * scale;
...
// Dragger类需要调整为支持pjax后的事件监听
function Dragger(options) {
    this.startElement = options.startElement;
    this.onPointerDown = options.onPointerDown || function() {};
    this.onPointerMove = options.onPointerMove || function() {};
    this.onPointerUp = options.onPointerUp || function() {};

    // 使用委托方式绑定事件，以便pjax加载后仍然有效
    document.addEventListener(downEvent, this);
}
